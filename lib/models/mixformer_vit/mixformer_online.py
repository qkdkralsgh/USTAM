...
class Attention(nn.Module):
    ...
    def forward(self, x, t_h, t_w, s_h, s_w, pre_attn=None, last_layer=None):
        """
        ...
        #     attn_map[:, :, :, 128:] = attn_map[:, :, :, 128:].clone()*pre_attn  ### vit-base
            # attn_map[:, :, :, 288:] = attn_map[:, :, :, 288:].clone()*pre_attn  ### vit-large
...
class SpatialFilter(nn.Module):
    def __init__(self, num_heads, spatial_dim=None):
        ...
        #         torch.zeros((1, num_heads, spatial_dim),  device='cuda', requires_grad=spatial_filter),
    ...
    def forward(self, batch, attn_map):
        ...
        # viz_bias = torch.mean(spatial_filter, dim=1)
        # for cnt, i in enumerate(spatial_filter[0]):
        #     plt.imshow(i.view(14, 14).detach().cpu().numpy(), cmap='hot', interpolation='nearest')
        #     cb = plt.colorbar()
        #     plt.savefig('/home/adminuser/Minho/SwinTrack/output/eval_viz/ssl_swint_result/bias_%d.jpg' % cnt)
        #     cb.remove()
...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward(self, x_t, x_ot, x_s, pre_attn):
        ...
        for i, blk in enumerate(self.blocks):
            if i == 0:
                x, _ = blk(x, H_t, W_t, H_s, W_s, pre_attn=pre_attn)
            elif i+1 != len(self.blocks):
                x, _ = blk(x, H_t, W_t, H_s, W_s)
            else:
                x, attn_map = blk(x, H_t, W_t, H_s, W_s, last_layer=True)
        ...
        return x_t_2d, x_ot_2d, x_s_2d, attn_map
...
class MixFormerOnlineScore(nn.Module):
    ...
    def forward(self, template, online_template, search, pre_attn, run_score_head=True, gt_bboxes=None):
        # search: (b, c, h, w)
        if template.dim() == 5:
        ...
        out, outputs_coord_new = self.forward_box_head(search) # , template, run_score_head, gt_bboxes)

        return out, outputs_coord_new, attn_map
