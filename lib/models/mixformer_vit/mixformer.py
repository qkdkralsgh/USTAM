...
from lib.test.analysis.engine import get_attention_map
import copy
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.):
        ...
        self.spatial_filter = SpatialFilter(num_heads, spatial_dim=324)  ### vit-base
        # self.spatial_filter = SpatialFilter(num_heads, spatial_dim=576)  ### vit-large

    def forward(self, x, t_h, t_w, s_h, s_w, pre_attn, last_layer=None):
        """
        ...
        attn = (q_s @ k.transpose(-2, -1)) * self.scale  # q_s : [B, 16(head), 576, 64],  k : [B, 16, 864, 64]
        attn = attn.softmax(dim=-1)  # [B, 16, 576, 864]
        
        attn_map = attn.clone()
        
        ### Temporal Filter ###
        # if pre_attn is not None:
        #     # attn_map[:, :, :, 128:] = attn_map[:, :, :, 128:].clone()*pre_attn  ### vit-base
        #     attn_map[:, :, :, 288:] = attn_map[:, :, :, 288:] + (attn_map[:, :, :, 288:].clone()*pre_attn)  ### vit-large
            
        ## Spatial Filter ###
        if last_layer:
            attn_map[:, :, :, 128:] = self.spatial_filter(B, attn_map[:, :, :, 128:].clone())  ### vit-base
        #     attn_map[:, :, :, 288:] = self.spatial_filter(B, attn_map[:, :, :, 288:].clone())  ### vit-large
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, attn_map[:, :, :, 128:]  ### vit-base 128, vit-large 288
...
class SpatialFilter(nn.Module):
    def __init__(self, num_heads, spatial_dim=None):
        super().__init__()
        self.num_heads = num_heads
        
        ### new version ### 
        self.spatial_filter = nn.Linear(spatial_dim, spatial_dim)
        
        ### old version ###
        # self.spatial_filter = nn.Parameter(
        #         torch.zeros((1, num_heads, spatial_dim),  device='cuda', requires_grad=True),
        #         requires_grad=True,
        # )
        # self.keep_threshold = nn.Parameter(
        #         torch.zeros((1, num_heads, 1),  device='cuda', requires_grad=spatial_filter),
        #         requires_grad=spatial_filter,
        # )
        
        
    def forward(self, batch, attn_map):
        ### old version ### 
        # spatial_filter = self.spatial_filter.expand(batch, self.num_heads, -1)
        # total_threshold = self.keep_threshold.expand(batch, self.num_heads, -1)  # [B, 1]
        
        cls_attn_token = torch.mean(attn_map, dim=-2)  # [B, 8, 196]
        max_attn_scores, _ = torch.max(cls_attn_token, dim=-1, keepdim=True)  # [B, 8, 1]
        
        ### new new version ###
        relative_attn_scores = cls_attn_token / max_attn_scores  # [B, 8, 196]
        
        ### final version ###
        # one_mat = torch.ones(cls_attn_token.size(-1), cls_attn_token.size(-1))
        # multi_one_mat = one_mat.expand(batch, self.num_heads, -1, -1).cuda()
        # max_mat = max_attn_scores.unsqueeze(-2) * multi_one_mat

        ### old version ###
        # total_threshold = total_threshold + relative_attn_scores.mean(dim=-1).view(batch, self.num_heads, 1)
        # spatial_filter = (spatial_filter + relative_attn_scores - total_threshold).softmax(dim=-1)
        # spatial_attn = attn_map + spatial_filter.unsqueeze(-1)
        
        ### new version ### 
        # spatial_filter = self.spatial_filter(relative_attn_scores.softmax(dim=-1))  ## Edit by Minho 23.07.03
        # spatial_attn = attn_map + spatial_filter.unsqueeze(-1)
        
        ### new new version ###
        spatial_filter = self.spatial_filter((attn_map.clone()-relative_attn_scores.unsqueeze(-1)).softmax(dim=-1))
        
        ### final version ###
        # spatial_filter = self.spatial_filter((attn_map.clone()-cls_attn_token.unsqueeze(-1)).softmax(dim=-1))
        
        spatial_attn = attn_map + spatial_filter
...
class Block(nn.Module):
    ...
    # ### Modify similarity measure ###
    # def forward(self, x, t_h, t_w, s_h, s_w, pre_attn, last_layer=None):
    #     attn_output, attn_map = self.attn(self.norm1(x), t_h, t_w, s_h, s_w, pre_attn, last_layer=last_layer)
    #     x = x + self.drop_path1(x*attn_output).softmax(dim=-1)
...
class VisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward(self, x_t, x_ot, x_s, pre_attn):
        """
        ...
        for i, blk in enumerate(self.blocks):
            if i == 0:  ### Edit by Minho 2023.08.01 : Temporal attention ablation study
                x, _ = blk(x, H_t, W_t, H_s, W_s, pre_attn)
            elif i < len(self.blocks)-1:
                x, _ = blk(x, H_t, W_t, H_s, W_s, None)
            else:
                x, attn_map = blk(x, H_t, W_t, H_s, W_s, None, last_layer=True)
        ...
        return x_t_2d, x_ot_2d, x_s_2d, attn_map  # , temporal_attn_map
...
class MixFormer(nn.Module):
    ...
    def forward(self, template, online_template, search, pre_attn, run_score_head=False, gt_bboxes=None):
        # search: (b, c, h, w)
        if template.dim() == 5:
        ...
        template, online_template, search, attn_map = self.backbone(template, online_template, search, pre_attn)
        # search shape: (b, 384, 20, 20)
        # Forward the corner head
        out_dict, outputs_coord_new = self.forward_box_head(search)
        
        return out_dict, outputs_coord_new, attn_map
