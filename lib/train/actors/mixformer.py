...
class MixFormerActor(BaseActor):
    ...
    def forward_pass(self, data, run_score_head):  ### Edit by Minho 2023.08.26
        search_bboxes = box_xywh_to_xyxy(data['search_anno'][0].clone())
        out_dict, _, _ = self.net(data['template_images'][0], data['template_images'][0], data['search_images'], None,
    ...
    # def forward_pass(self, data, run_score_head):  ### Edit by Minho 2023.08.26
    #     search_bboxes = box_xywh_to_xyxy(data['search_anno'][0].clone())
    #     out_dict, _, _ = self.net(data['template_images'][0], data['template_images'][1], data['search_images'], None,
    #                            run_score_head=run_score_head, gt_bboxes=search_bboxes)
    #     # out_dict: (B, N, C), outputs_coord: (1, B, N, C), target_query: (1, B, N, C)
    #     return out_dict
