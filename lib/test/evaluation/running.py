...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['trackingnet', 'got_10k']:
...
def run_sequence(seq: Sequence, tracker: Tracker, debug=False, viz_attn=False, num_gpu=8):
    ...
    if debug:
        output = tracker.run_sequence(seq, debug=debug, viz_attn=viz_attn)
    else:
        try:
            output = tracker.run_sequence(seq, debug=debug, viz_attn=viz_attn)
...
def run_dataset(dataset, trackers, debug=False, viz_attn=False, threads=0, num_gpus=8):
    ...
    if mode == 'sequential':
        for seq in dataset:
            for tracker_info in trackers:
                run_sequence(seq, tracker_info, debug=debug, viz_attn=viz_attn)
    elif mode == 'parallel':
        param_list = [(seq, tracker_info, debug, viz_attn, num_gpus) for seq, tracker_info in product(dataset, trackers)]
