...
class MixFormer(BaseTracker):
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            out_dict, _, attn_map = self.network(self.template, self.online_template, search, pre_attn=self.pre_attn)
        
        ### Temporal filter ###
        self.pre_attn = attn_map
        ...
        # update template  ### Edit by Minho 2023.07.07
        ...
        # for debug  ### Edit by Minho 2023.07.13
        if self.debug:
            x1, y1, w, h = self.state
            image_BGR = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            cv2.rectangle(image_BGR, (int(x1),int(y1)), (int(x1+w),int(y1+h)), color=(0,0,255), thickness=2)
            save_path = os.path.join(self.save_dir, "%s/" % folder_name)
            Path(save_path).mkdir(parents=True, exist_ok=True)
            save_file = os.path.join(save_path, "%04d.jpg" %  self.frame_id)
        ...
        if self.viz_attn:
            ...
            Path(save_path).mkdir(parents=True, exist_ok=True)
            save_file = os.path.join(save_path, "%04d.jpg" %  self.frame_id)
            attention_image = get_attention_map(search_BGR, [attn_map], get_decoder=True)#,get_mask=True)
            cv2.imwrite(save_file, attention_image)
        if self.save_all_boxes:
