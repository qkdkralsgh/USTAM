...
from lib.models.mixformer_vit.mixformer import Attention
...
def evaluate(model, template, search, skip=200, display_info='MixFormer'):
    ...
    # pre_attn = torch.rand([1, 16, 576, 576]).cuda()
    pre_attn = torch.rand([1, 12, 324, 324]).cuda()
    custom_ops = {Attention: get_complexity_Attention}
    macs, params = profile(model, inputs=(template, template, search, pre_attn, False),  ### Edit by Minho 2023.07.29
    ...
    with torch.no_grad():
        # overall
        for i in range(T_w):
            _ = model(template, template, search, None, run_score_head=True)
        start = time.time()
        for i in range(T_t):
            if i % skip == 0:
...
if __name__ == "__main__":
    ...
    model_module = importlib.import_module('lib.models.%s.mixformer_online' % args.script)
    # if args.script == "mixformer_online":
    if args.script == "mixformer_cvt":
        model_constructor = model_module.bulid_mixformer_cvt_online_score
    elif args.script == "mixformer_vit":
        model_constructor = model_module.build_mixformer_vit_online_score
